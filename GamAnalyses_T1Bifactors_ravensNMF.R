...
NmfModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + mood_4factorv2 + psychosis_4factorv2 + externalizing_4factorv2 + phobias_4factorv2 + overall_psychopathology_4factorv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
#Pull p-values
p_mood <- sapply(NmfModels, function(v) summary(v)$p.table[3,4])
...
#Pull p-values
p_ext <- sapply(NmfModels, function(v) summary(v)$p.table[5,4])
...
#Pull p-values
p_fear <- sapply(NmfModels, function(v) summary(v)$p.table[6,4])
...
#Pull p-values
p_overall <- sapply(NmfModels, function(v) summary(v)$p.table[7,4])
