...
#Load data
data.FS <- readRDS("/data/jux/BBL/projects/pncT1AcrossDisorder/subjectData/n1394_T1_subjData.rds")

#Load library
library(mgcv)

#Get FS_SA variable names 
fsComponents <- names(data.FS)[grep("_thickavg",names(data.FS))]

#Run gam models (GAM without TBV)
FsModels <- lapply(fsComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + mood_4factorv2 + psychosis_4factorv2 + externalizing_4factorv2 + phobias_4factorv2 + overall_psychopathology_4factorv2, list(i = as.name(x))), method="REML", data = data.FS)
})

#Look at model summaries
models <- lapply(FsModels, summary)
...
#Pull p-values
p_mood <- sapply(FsModels, function(v) summary(v)$p.table[4,4])
...
mood_coeff <- models[as.numeric(Fs_mood_fdr)]
...
#Pull p-values
p_psy <- sapply(FsModels, function(v) summary(v)$p.table[5,4])
...
#To check direction of coefficient estimates
psy_coeff <- models[as.numeric(Fs_psy_fdr)]
...
#Pull p-values
p_ext <- sapply(FsModels, function(v) summary(v)$p.table[6,4])
...
#To check direction of coefficient estimates
ext_coeff <- models[as.numeric(Fs_ext_fdr)]
...
#Pull p-values
p_fear <- sapply(FsModels, function(v) summary(v)$p.table[7,4])
...
#To check direction of coefficient estimates
fear_coeff <- models[as.numeric(Fs_fear_fdr)]
...
#Pull p-values
p_overall <- sapply(FsModels, function(v) summary(v)$p.table[8,4])
...
#List the FS components that survive FDR correction
Fs_overall_fdr <- row.names(p_overall_fdr)[p_overall_fdr<0.05]

#Name of the FS components that survive FDR correction
Fs_overall_fdr_names <- fsComponents[as.numeric(Fs_overall_fdr)]

#To check direction of coefficient estimates
overall_coeff <- models[as.numeric(Fs_overall_fdr)]
overall_p.table_list <- lapply(overall_coeff,`[[`, 'p.table')
