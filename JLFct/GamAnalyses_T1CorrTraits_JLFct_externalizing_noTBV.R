...
JlfModels <- lapply(jlfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + externalizing_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.JLF)
})
...
#Pull p-values
p_ext <- sapply(JlfModels, function(v) summary(v)$p.table[3,4])
