...
NmfModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ sex + averageManualRating + mood_4factorv2 + psychosis_4factorv2 + externalizing_4factorv2 + phobias_4factorv2 + overall_psychopathology_4factorv2 + phobias_4factorv2*sex , list(i = as.name(x))), method="REML", data = data.NMF)
})
