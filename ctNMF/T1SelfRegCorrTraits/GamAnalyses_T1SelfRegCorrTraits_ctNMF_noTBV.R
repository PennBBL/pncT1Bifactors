...
NmfModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + mood_sr_corrtraits + psychosis_sr_corrtraits + externalizing_sr_corrtraits + fear_sr_corrtraits, list(i = as.name(x))), method="REML", data = data.NMF)
})
