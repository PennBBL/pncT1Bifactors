...
NmfModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + mprage_antsCT_vol_TBV + fear_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
#Pull p-values
p_fear <- sapply(NmfModels, function(v) summary(v)$p.table[4,4])
