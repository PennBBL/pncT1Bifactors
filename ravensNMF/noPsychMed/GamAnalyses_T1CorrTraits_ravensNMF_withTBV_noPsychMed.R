...
NmfMoodModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + mprage_antsCT_vol_TBV + mood_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
NmfPsyModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + mprage_antsCT_vol_TBV + psychosis_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
NmfExtModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + mprage_antsCT_vol_TBV + externalizing_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
NmfFearModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + mprage_antsCT_vol_TBV + fear_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
#Pull p-values
p_mood <- sapply(NmfMoodModels, function(v) summary(v)$p.table[5,4])
...
#Pull p-values
p_psy <- sapply(NmfPsyModels, function(v) summary(v)$p.table[5,4])
...
#Pull p-values
p_ext <- sapply(NmfExtModels, function(v) summary(v)$p.table[5,4])
...
#Pull p-values
p_fear <- sapply(NmfFearModels, function(v) summary(v)$p.table[5,4])
