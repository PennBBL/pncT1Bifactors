...
#Load data
data.NMF <- readRDS("/data/jux/BBL/projects/pncT1AcrossDisorder/subjectData/n1240_T1_subjData_NoPsychMeds.rds")
...
NmfModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + mprage_antsCT_vol_TBV + mood_4factorv2 + psychosis_4factorv2 + externalizing_4factorv2 + phobias_4factorv2 + overall_psychopathology_4factorv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
Nmf_mood_uncorrected_p <- row.names(p_mood_round)[p_mood_round<0.05]

#Names of regions with significant uncorrected p values
Nmf_mood_uncorrected_p_names <- nmfComponents[as.numeric(Nmf_mood_uncorrected_p)]
...
#Pull p-values
p_psy <- sapply(NmfModels, function(v) summary(v)$p.table[6,4])
...
#List the NMF components with significant uncorrected p values
Nmf_psy_uncorrected_p <- row.names(p_psy_round)[p_psy_round<0.05]

#Names of regions with significant uncorrected p values
Nmf_psy_uncorrected_p_names <- nmfComponents[as.numeric(Nmf_psy_uncorrected_p)]
...
#Pull p-values
p_ext <- sapply(NmfModels, function(v) summary(v)$p.table[7,4])
...
#List the NMF components with significant uncorrected p values
Nmf_ext_uncorrected_p <- row.names(p_ext_round)[p_ext_round<0.05]

#names of regions with significant uncorrected p values
Nmf_ext_uncorrected_p_names <- nmfComponents[as.numeric(Nmf_ext_uncorrected_p)]
...
#List the NMF components with significant uncorrected p values
Nmf_fear_uncorrected_p <- row.names(p_fear_round)[p_fear_round<0.05]

#Names of regions with significant uncorrected p values
Nmf_fear_uncorrected_p_names <- nmfComponents[as.numeric(Nmf_fear_uncorrected_p)]
...
#List the NMF components with significant uncorrected p values
Nmf_overall_uncorrected_p <- row.names(p_overall_round)[p_overall_round<0.05]

#Names of regions with significant uncorrected p values
Nmf_overall_uncorrected_p_names <- nmfComponents[as.numeric(Nmf_overall_uncorrected_p)]
