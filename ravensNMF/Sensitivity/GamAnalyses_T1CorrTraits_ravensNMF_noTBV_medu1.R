...
NmfMoodModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + medu1 + mood_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
NmfPsyModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + medu1 + psychosis_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
NmfExtModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + medu1 + externalizing_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
