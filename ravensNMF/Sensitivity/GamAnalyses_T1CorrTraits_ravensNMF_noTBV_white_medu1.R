...
NmfPsyModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + white + medu1 + psychosis_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
NmfExtModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + white + medu1 + externalizing_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
NmfFearModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + white + medu1 + fear_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
