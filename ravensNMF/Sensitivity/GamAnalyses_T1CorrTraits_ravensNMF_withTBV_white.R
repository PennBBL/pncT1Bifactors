...
NmfMoodModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + mprage_antsCT_vol_TBV + white + mood_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
NmfPsyModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + mprage_antsCT_vol_TBV + white + psychosis_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
NmfExtModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + mprage_antsCT_vol_TBV + white + externalizing_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
...
NmfFearModels <- lapply(nmfComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + mprage_antsCT_vol_TBV + white + fear_corrtraitsv2, list(i = as.name(x))), method="REML", data = data.NMF)
})
