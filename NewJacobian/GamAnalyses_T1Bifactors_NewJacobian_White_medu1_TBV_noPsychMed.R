...
JacModels <- lapply(jacComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + white + medu1 + mprage_antsCT_vol_TBV + mood_4factorv2 + psychosis_4factorv2 + externalizing_4factorv2 + phobias_4factorv2 + overall_psychopathology_4factorv2, list(i = as.name(x))), method="REML", data = data.JAC)
})
...
#Pull p-values
p_mood <- sapply(JacModels, function(v) summary(v)$p.table[7,4])
...
#Pull p-values
p_psy <- sapply(JacModels, function(v) summary(v)$p.table[8,4])
...
#Pull p-values
p_ext <- sapply(JacModels, function(v) summary(v)$p.table[9,4])
...
#Pull p-values
p_fear <- sapply(JacModels, function(v) summary(v)$p.table[10,4])
...
#Pull p-values
p_overall <- sapply(JacModels, function(v) summary(v)$p.table[11,4])
