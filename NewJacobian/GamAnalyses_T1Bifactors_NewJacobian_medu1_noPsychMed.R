...
JacModels <- lapply(jacComponents, function(x) {
  gam(substitute(i ~ s(age) + sex + averageManualRating + medu1 + mood_4factorv2 + psychosis_4factorv2 + externalizing_4factorv2 + phobias_4factorv2 + overall_psychopathology_4factorv2, list(i = as.name(x))), method="REML", data = data.JAC)
})
...
mood_coeffs <- sapply(JacModels, function(v) summary(v)$p.table[5,])
overall_coeffs <- sapply(JacModels, function(v) summary(v)$p.table[9,])

#################################################
#### Pulling out values for the result table ####
#################################################

#Pull B (standard error) for CT_fear
B_mood <- as.data.frame(mood_coeffs[1,])

#Print original p-values to two decimal places
B_mood_round <- round(B_mood,2)

#Pull SE (standard error) for CT_fear
...
#Pull p-values
p_mood <- sapply(JacModels, function(v) summary(v)$p.table[5,4])
...
#Pull p-values
p_psy <- sapply(JacModels, function(v) summary(v)$p.table[6,4])
...
#Pull p-values
p_ext <- sapply(JacModels, function(v) summary(v)$p.table[7,4])
...
#Pull p-values
p_fear <- sapply(JacModels, function(v) summary(v)$p.table[8,4])
...
#Pull p-values
p_overall <- sapply(JacModels, function(v) summary(v)$p.table[9,4])
