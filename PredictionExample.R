...
#Remove NAs (three subjects are missing cognitive data on all three cog factors) {CAN PROBABLY REMOVE - THIS WAS SPECIFIC TO MY MEASURES WHICH HAD MISSING DATA}
...
#CT components predicting Fear {UPDATE MODEL - ASK TED WHAT YOUR COVARIATES SHOULD BE}
...
{NULL IS THE EXACT SAME VARIABLES EXCEPT NO NMF COMPONENTS}
...
#Correlation between actual fear scores and predicted fear scores in a model with age, sex, cognition, and the 18 NMF networks {YOU WANT TO SEE THAT THIS IS A HIGHER CORRELATION THAN THE NULL MODEL CORRELATION}
nmfCor <- cor.test(predict(NmfModel), data.NMF$phobias_4factorv2)

#Compare the null model to the Nmf model with an F test {YOU WANT THIS TO BE SIGNIFICANT = SAYS THERE IS A SIGNIFICANT DIFFERENCE BETWEEN YOUR NULL AND FULL MODELS}
Ftest <- anova(NmfModel,NullModel)

#Plot predicted fear score vs actual fear scores. {UPDATE PATHS; CAN PLAY AROUND WITH COLORS TOO}
