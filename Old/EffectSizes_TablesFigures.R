...
#SAVE TABLE
write.csv(table1,"/data/jux/BBL/projects/pncT1AcrossDisorder/TablesFigures/Table_DemographicsByDiagnosis.csv",row.names=TRUE,quote=FALSE)
...
ggsave(file="/data/jux/BBL/projects/pncT1AcrossDisorder/TablesFigures/Figure_BifactorsByDiag.png", width = 15, height = 5, units = "in", dpi = 300)
...
##First, define lists of variables to test
#Global measures
Vol_GmWmCsf <- subjData[,grep("mprage_antsCT_vol",names(subjData))]
Vol_GmWmCsf_List <- colnames(Vol_GmWmCsf)

#Lobes
Vol_Lobes <- subjData[,grep("Lobe_WM",names(subjData))]
Vol_Lobes <- subjData[,grep("Lobe_WM",names(subjData))]
result <- a[ grep("blue", a$x) , ]
Vol_Lobe_List <- colnames(Vol_Lobes)
