...
#Load subject GO1 data release (n=1601), T1 QA dataset (n=1601), GMD/CT data (n=1570 because excludes those with missing MARS vol data), correlated traits data (n=9361; one subject missing corr traits data), and compartment volumes (cortical GM, subcortical GM, WM, cerebellum, brainstem) from the new ANTsCT pipeline (n=1601). 
...
data.qa <- read.csv("/data/joy/BBL/projects/pncT1AcrossDisorder_Kaczkurkin/subjectData/pnc1601_qaRatings_20160316.csv", header=TRUE, na.strings=".")
data.GmdCT <- read.csv("/data/joy/BBL/projects/pncT1AcrossDisorder_Kaczkurkin/subjectData/n1601_GMD_CT_vals.csv", header=TRUE, na.strings=".")
...
data.CompartVol <- read.csv("/data/joy/BBL/projects/pncT1AcrossDisorder_Kaczkurkin/subjectData/go1AntsCtVolumes.csv", header=TRUE, na.strings = ".")
...
data.merge2 <- merge(data.merge, data.GmdCT, by=c("bblid","scanid"))
data.merge3 <- merge(data.merge2, data.CorrTraits, by="bblid")
data.merge4 <- merge(data.merge3, data.CompartVol, by=c("bblid","scanid"))

#Put bblids in ascending order
data.final <- data.merge4[order(data.merge4$bblid),]
...
data.final$ACROSS.INCLUDE[data.final$mprageSbiaExclude==1]<-0
...
##After exclusion criteria, there are 1385 subjects
subjData<-data.final
...
#ALL ROIS
dataCT<-subjData[,grep("mprage_mars_ct",names(subjData))]

#GM-- these are the gray matter regions-- would only FDR across these 119 regions
dataCTGm<-dataCT[,-(unique(c( grep("WM",names(dataCT)), grep("Vent",names(dataCT)), grep("White",names(dataCT)), grep("corpus",names(dataCT)), grep("DC",names(dataCT)), 
        grep("Brain_Stem",names(dataCT)), grep("CSF",names(dataCT)), grep("InC",names(dataCT)), grep("fornix",names(dataCT)), grep("ped",names(dataCT)), grep("Vessel",names(dataCT)) )))]

#WM-- these are the white matter regions: 17 regions
dataCTWm<-dataCT[,(unique(c( grep("WM",names(dataCT)),grep("fornix",names(dataCT)),grep("White_Matter",names(dataCT)), grep("corpus",names(dataCT)), grep("InC",names(dataCT)), 
      grep("ped",names(dataCT)) )))]

#CSF
dataCTCsf<-dataCT[,c(grep("CSF",names(dataCT)),grep("Vent",names(dataCT)))]
dataCTCsf<-dataCTCsf[,-c(grep("DC",names(dataCTCsf)))]

#Get total volume per tissue class-- these define tissue class
dataCTGm<-data.matrix(dataCTGm)
subjData$CT_gmTotal<-rowSums(dataCTGm)
dataCTWm<-data.matrix(dataCTWm)
subjData$CT_wmTotal<-rowSums(dataCTWm)
dataCTCsf<-data.matrix(dataCTCsf)
subjData$CT_csfTotal<-rowSums(dataCTCsf)


###GMD###

#ALL ROIS
dataGMD<-subjData[,grep("mprage_mars_gmd",names(subjData))]
...
#WM-- these are the white matter regions: 17 regions
dataGMDWm<-dataGMD[,(unique(c( grep("WM",names(dataGMD)),grep("fornix",names(dataGMD)),grep("White_Matter",names(dataGMD)), grep("corpus",names(dataGMD)), grep("InC",names(dataGMD)), 
       grep("ped",names(dataGMD)) )))]

#CSF
dataGMDCsf<-dataGMD[,c(grep("CSF",names(dataGMD)),grep("Vent",names(dataGMD)))]
dataGMDCsf<-dataGMDCsf[,-c(grep("DC",names(dataGMDCsf)))] 

#Get total volume per tissue class-- these define tissue class
dataGMDGm<-data.matrix(dataGMDGm)
subjData$GMD_gmTotal<-rowSums(dataGMDGm)
dataGMDWm<-data.matrix(dataGMDWm)
subjData$GMD_wmTotal<-rowSums(dataGMDWm)
dataGMDCsf<-data.matrix(dataGMDCsf)
subjData$GMD_csfTotal<-rowSums(dataGMDCsf)
...
#cerebellum
dataCTCereb<-dataCT[,c(grep("CerExt",names(dataCT)),grep("CerVerLob",names(dataCT)))] #5 as per atlas
...
dataCTSubcort<-dataCT[,c(grep("Caudate",names(dataCT)),grep("Putamen",names(dataCT)),grep("Pallidum",names(dataCT)),grep("Accumbens",names(dataCT)),grep("Thal",names(dataCT)),
        grep("BasForebr",names(dataCT)),grep("Hippo",names(dataCT)),grep("Amyg",names(dataCT)))] #16 regions
...
dataCTCereb<-data.matrix(dataCTCereb)
subjData$CT_gmCerebTotal<-rowSums(dataCTCereb)
...
#cerebellum
dataGMDCereb<-dataGMD[,c(grep("CerExt",names(dataGMD)),grep("CerVerLob",names(dataGMD)))] #5 as per atlas
...
saveRDS(subjData,"/data/joy/BBL/projects/pncT1AcrossDisorder_Kaczkurkin/subjectData/n1385_MARS_datarel_020716.rds")
